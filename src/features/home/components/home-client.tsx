...
export default function HomePageClient({ latestArticles }: { latestArticles: Article[] }) {
    ...
    // ИСПРАВЛЕННАЯ логика отслеживания скролла
    useEffect(() => {
        const handleScroll = () => {
            const scrollPosition = window.scrollY + 100; // Смещение для лучшего определения
            
            const sections = ['hero', 'video', 'collections', 'articles'];
            let currentSection = 'hero';

            // Находим секцию, которая ближе всего к верху экрана
            for (let i = sections.length - 1; i >= 0; i--) {
                ...
                if (element) {
                    ...
                    // Если прокрутка дошла до начала секции
            ...
            // Дополнительная проверка: если мы в самом низу страницы, показываем последнюю секцию
