// Файл: src/components/ArticleForm.tsx
...
export default function ArticleForm({ initialData }: { initialData?: Article }) {
    ...
    // Инициализируем состояния из initialData или оставляем пустыми
    const [title, setTitle] = useState(initialData?.title || '');
    const [slug, setSlug] = useState(initialData?.slug || '');
    const [category, setCategory] = useState(initialData?.category || 'Сведение');
...
const handleContentChange = (index: number, field: string, value: string | string[]) => {
        ...
        const blockToUpdate = { ...newContent[index] }; // Создаем копию блока для изменения

        // Используем явные проверки, чтобы TypeScript "понимал", с каким типом блока мы работаем
        switch (blockToUpdate.type) {
            case 'heading':
            case 'paragraph':
            ...
            case 'quote':
                if (field === 'text' && typeof value === 'string') {
                    blockToUpdate.text = value;
                } else if (field === 'author' && typeof value === 'string') {
        ...
        newContent[index] = blockToUpdate; // Заменяем старый блок на обновленный
    ...
    const handleFormSubmit = async (event: React.FormEvent) => {
        ...
        // Собираем данные в объект, соответствующий типу Article
        const articleData: Article = {
            title, slug, category, content,
            // Если редактируем, сохраняем старые данные, иначе - создаем новые
            author: initialData?.author || { name: "Admin", avatarUrl: "/images/author.jpg" },
            publishedDate: initialData?.publishedDate || new Date().toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' }),
            readingTime: Math.ceil(JSON.stringify(content).length / 1500),
        ...
        <form onSubmit={handleFormSubmit} className="space-y-8 max-w-4xl">
            ...
            <div className="p-6 rounded-2xl border border-white/10 bg-white/5 space-y-6">
                ...
                <div>
                    ...
                    {/* Поле slug нередактируемо для существующих статей */}
                    <input type="text" value={slug} onChange={e => setSlug(e.target.value)} className="input-style" required readOnly={isEditing} />
                </div>
                <div>
                ...
                {content.map((block, index) => (
                    <div key={index} className="rounded-2xl border border-white/10 bg-white/5">
                        ...
                        <div className="p-4 space-y-3">
                            ...
                            { block.type === 'youtube' && <>
                                <input type="text" value={block.videoId} placeholder="YouTube Video ID..." onChange={e => handleContentChange(index, 'videoId', e.target.value)} className="input-style" />
